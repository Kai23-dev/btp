...
def process_yearly_data(daily, year):
    # daily is expected to be a dict with lists similar to Open-Meteo daily response
    time = daily.get('time', [])
...
def calculate_pmp_with_eva(amdp_values):
    ...
    if rest:
        ...
        S = calculate_std(rest)
    else:
        Xmean = mean
        S = std

    if S > 0:
        frequency_factor = (Xmax - Xmean) / S
        frequency_factor = max(5, min(15, frequency_factor))
    else:
        frequency_factor = 10
