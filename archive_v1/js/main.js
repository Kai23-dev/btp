/**
 * HydroIntel Pro — Main Application Logic
 * BTP-2 | IIT Kharagpur
 *
 * PMP METHOD: Hershfield Envelope Curve (Sarkar & Maity, 2020, MethodsX)
 *   - Km is read from a location-fixed envelope curve based on mean AMDP
 *   - This gives a stable PMP that does NOT change with the selected year range
 *   - Reference: Hershfield (1961), adapted by Sarkar & Maity (2020)
 *
 * LSTM: Khan, Sarkar & Maity (2023) — 5-yr window, 3-yr horizon
 * CNN : Maity, Srivastava, Sarkar & Khan (2024)
 * DATA: Open-Meteo Historical Weather Archive API (open-source, free to use)
 */

// ── App State ──────────────────────────────────────────────
let map, mapMarker;
let charts = {};
let globalData    = [];   // full 1990-present record — used ONLY for PMP calculation
let selectedData  = [];   // user's chosen period — drives charts, table & LSTM
let currentResults = null;
let lstmChartRef   = null;

// Fixed PMP record window — always 1990 to (current year - 1)
// Physically, PMP is a location property, not a function of the analysis window.
// Ref: Sarkar & Maity (2020) — PMP should use the longest available record.
...
function loadDemoData() {
    ...
    processAndRender(7 / 100);

    document.getElementById('resultsSection').classList.remove('d-none');
    document.getElementById('exportBtn').disabled = false;
    showError('Demo mode: Mumbai data (2010-2023). PMP uses the full record; chart/LSTM use your selected period.');
}
...
async function onFormSubmit(e) {
    ...
    try {
        const fullEnd = new Date().getFullYear() - 1;

        // Fetch full record for PMP (always 1990-present, location-fixed)
        // and the user's selected period for charts/LSTM — in parallel.
        const [fullData, selData] = await Promise.all([
            fetchYearRange(lat, lon, PMP_RECORD_START, fullEnd),
            fetchYearRange(lat, lon, startYear, endYear)
        ]);

        if (selData.length === 0) {
            throw new Error('No data received for the selected period. The API may be rate-limited — wait a few minutes and retry, or use "Load Demo".');
        }

        // globalData drives PMP; selectedData drives charts/LSTM
        globalData   = fullData.length > 0 ? fullData : selData;
        selectedData = selData;

        processAndRender(climAdj);
    } catch (err) {
...
// ── Open-Meteo Fetch ───────────────────────────────────────
async function fetchYearRange(lat, lon, startYear, endYear) {
    const results = [];
        ...
        for (let attempt = 0; attempt < 3 && !success; attempt++) {
            try {
                const url = `https://archive-api.open-meteo.com/v1/archive?` +
                    ...
                    `precipitation_sum,windspeed_10m_max&timezone=auto`;
                ...
                if (!data.daily || !data.daily.time) break;

                const row = processYear(data.daily, y);
                if (row) results.push(row);
                success = true;
                await delay(300); // stay within free API limits
            } catch (_) { break; }
        }
    }
...
function processYear(daily, year) {
    const precip   = daily.precipitation_sum.filter(v => v !== null && v >= 0);
    const tMean    = daily.temperature_2m_mean.filter(v => v !== null);
    const tMax     = daily.temperature_2m_max.filter(v => v !== null);
    const tMin     = daily.temperature_2m_min.filter(v => v !== null);
    const wind     = daily.windspeed_10m_max.filter(v => v !== null);

    if (precip.length === 0) return null;

    return {
        year:       parseInt(year),
        amdp:       Math.max(...precip),
        totalPrecip: precip.reduce((a, b) => a + b, 0),
        avgTemp:    tMean.length ? mean(tMean) : null,
        maxTemp:    tMax.length  ? Math.max(...tMax) : null,
        minTemp:    tMin.length  ? Math.min(...tMin) : null,
        avgWind:    wind.length  ? mean(wind) : null,
        dataPoints: precip.length
    };
}
